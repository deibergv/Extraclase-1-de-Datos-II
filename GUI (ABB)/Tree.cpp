...
void Tree<T>::Insertar(const int dat)
{
    ...
    if(Vacio(padre)) raiz = new Node<T>(dat);
        // Si el int es menor que el que contiene el nodo padre, se inserta en la rama izquierda
    else if(dat < padre->dato) padre->izquierdo = new Node<T>(dat);
        // Si el int es mayor que el que contiene el nodo padre, se inserta en la rama derecha
